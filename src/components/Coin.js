      ...
      <div className="flex justify-between w-full mx-4 items-center h-full border-b-2 border-gray-200 w-80% ">
        <div className="flex items-center pr-6 min-w-min h-20 ">
          ...
          <div className=" flex-col  ">
            <h1 className="h-8 mr-2 text-base w-40 align-center ">{name}</h1>
        ...
        <div className=" flex ">
          {priceChange < 0 ? (
            <p className="red text-red-500 flex"> <TiArrowSortedDown className="h-fit w-full px-2 pt-1"/>{priceChange.toFixed(2)}%</p>
          ) : (
            <p className="green text-green-500"><TiArrowSortedUp className="h-fit w-full px-2 pt-1"/>{priceChange.toFixed(2)}%</p>
          )}
        </div>
      </div>
    </div>
